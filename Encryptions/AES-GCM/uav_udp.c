// Server 1 (uav_udp) using AES-GCM with Rotating Nonce Storage
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
...
void generate_nonce(unsigned char *nonce) {
    memset(nonce, 0, AES_GCM_IV_SIZE);                         // Clear the nonce buffer
...
int main() {
    ...
    unsigned char send_buffer[AES_GCM_IV_SIZE + MESSAGE_LEN + AES_GCM_TAG_SIZE];
    int send_len;
    ...
    while (1) {
        ...
        if (FD_ISSET(sockfd1, &readfds)) {
            ...
            EVP_EncryptUpdate(ctx, send_buffer + AES_GCM_IV_SIZE, &send_len, buffer, n);
            EVP_EncryptFinal_ex(ctx, send_buffer + AES_GCM_IV_SIZE + send_len, &n);
            send_len += n;
            ...
            // Prepare packet by copying nonce and tag
            memcpy(send_buffer, nonce, AES_GCM_IV_SIZE);
            memcpy(send_buffer + AES_GCM_IV_SIZE + send_len, tag, AES_GCM_TAG_SIZE);

            // Final length includes nonce, ciphertext, and tag
            send_len += AES_GCM_IV_SIZE + AES_GCM_TAG_SIZE;
            store_nonce(nonce);

            // Send encrypted message (nonce + ciphertext + tag) to Server 2
            sendto(sockfd1, send_buffer, send_len, MSG_CONFIRM, (const struct sockaddr *)&server2_addr, sizeof(server2_addr));
        }

        if (FD_ISSET(sockfd2, &readfds)) {
            // Receive data from Server 2
            n = recvfrom(sockfd2, send_buffer, sizeof(send_buffer), MSG_WAITALL, (struct sockaddr *)&cliaddr, &len);

            // Extract nonce, ciphertext, and tag from received packet
            memcpy(nonce, send_buffer, AES_GCM_IV_SIZE);
            memcpy(tag, send_buffer + n - AES_GCM_TAG_SIZE, AES_GCM_TAG_SIZE);
            ...
            EVP_DecryptUpdate(ctx, decrypted, &decrypted_len, send_buffer + AES_GCM_IV_SIZE, n - AES_GCM_IV_SIZE - AES_GCM_TAG_SIZE);
